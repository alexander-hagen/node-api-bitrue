...
const
  ...
  accountUrl = 'wss://wsapi.bitrue.com'; // User Data Streams WebSocket
    ...
    this._ws.onmessage = msg => {
      ...
      switch(true) {
        ...
        case message.hasOwnProperty("e"):
          ...
          symbol=message.s;
          if (this._handles.has(method)) {
            this._handles.get(method).forEach((cb,i) => { cb(method,message,symbol,message.E); });
          } else {
            console.log('ws no handler', method,message);
          };
          break;

        default:
          console.log('ws unprocessed',message);
          break;
      };
...
var BitrueSocket = function(url, keys, token, gzip) {
  ...
  if(url) {
    if(token) { url+='/stream?listenKey='+token };
...
BitrueSocket.prototype.setHandler = async function(method, callback) {
  await this.socket.setHandler(method, callback);
};

BitrueSocket.prototype.clearHandler = async function(method) {
...
BitrueSocket.prototype.createListenKey = async function() {
  ...
  if(result.error) { throw result; };
  if(result.code==200) { this.renewInterval=setInterval(this.renewListenKey,1200000,result.data.listenKey,this); };
  return result;
};

BitrueSocket.prototype.renewListenKey = async function(key) {
...
BitrueSocket.prototype.deleteListenKey = async function(key) {
